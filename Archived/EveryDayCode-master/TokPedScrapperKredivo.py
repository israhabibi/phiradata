# usage : python TokPedScrapperKredivo.py "Keyword" page_to_scrape
# example : python TokPedScrapperKredivo.py "Samsung A51" 5
import requests
import re
import json
import time
...
# analyze tokopedia request, search for url containt json data
...
# get total data returned by keyword
        totalData = jsonData['header']['total_data']
        return totalData

# get detail information for each data, normalize json into tabular format, and create pandas dataframe from data
    def getProductData(self, totalData):
        ...
        for baris in range (0, totalData, 60):
            ...
            dataFinal = masterDF[['category_id','category_name','count_review','id','name','original_price','price','price_int','rating','shop.city','shop.id','shop.is_gold','shop.is_official','shop.is_power_badge','shop.location','shop.name','shop.reputation']]  
            dataFinal['get_date'] = date.today()
            dataFinal['keyword'] = self.keyword
        return dataFinal

# save pandas dataframe into csv
    def toCsv (self, dataFinal):
        dataFinal.to_csv(self.keyword+str(date.today())+'.csv')
    
    # save pandas dataframe into csv
    def toJson (self, dataFinal):
        dataFinal = dataFinal.reset_index(drop=True)
        dataFinal.to_json(self.keyword+str(date.today())+'.json', orient='records')
